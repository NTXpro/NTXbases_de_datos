CREATE PROC [ERP].[Usp_Ins_AsientoContable_CuentaPagar]
@IdAsiento INT OUT,
@IdComprobante INT,
@IdOrigen	INT,
@IdSistema INT
...
DECLARE @IdCuentaPagar INT = (SELECT IdCuentaPagar FROM ERP.AplicacionAnticipoPagar WHERE ID = @IdComprobante)
...
SELECT IdEmpresa,
@IdPeriodo,
(SELECT ERP.GenerarNombreAsientoCuentaPagar(@IdCuentaPagar)),
@IdOrigen,
@OrdenAsiento,
FechaAplicacion,
IdMoneda,
...
CAST(0 AS BIT), 
CAST(1 AS BIT)
FROM ERP.AplicacionAnticipoPagar
