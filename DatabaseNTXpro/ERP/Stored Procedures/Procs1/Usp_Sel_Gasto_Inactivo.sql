CREATE PROC [ERP].[Usp_Sel_Gasto_Inactivo] 
@IdEmpresa INT,
@IdAnio INT,
@IdMes INT
AS
BEGIN
		...
		WHERE GA.Flag = 0 AND GA.FlagBorrador = 0 AND GA.IdEmpresa = @IdEmpresa AND YEAR(GA.FechaEmision) = @IdAnio AND MONTH(GA.FechaEmision) = @IdMes
