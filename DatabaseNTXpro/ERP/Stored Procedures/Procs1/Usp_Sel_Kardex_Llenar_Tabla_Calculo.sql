...
-- =============================================
CREATE PROCEDURE [ERP].[Usp_Sel_Kardex_Llenar_Tabla_Calculo]
    @IdEmpresaTAB INT,
@IdEstablecimientoTAB INT,
@IdAlmacenTAB INT,
@IdMonedaTAB INT,
...
BEGIN
    -------------------------------
        ...
        INSERT INTO ERP.KardexRecalculo
                ...
                FROM ERP.Vale v
                    ...
                    VE.Abreviatura NOT IN ('A') AND
                        ...
                        V.Flag = 1 AND
                        --------------
                        V.IdEmpresa = @IdEmpresaTAB AND
                        P.IdEmpresa = @IdEmpresaTAB AND
                        E.ID = @IdEstablecimientoTAB AND
                        (@IdProyectoTAB = 0 OR V.IdProyecto = @IdProyectoTAB) AND
                        A.ID = @IdAlmacenTAB AND
                        CAST(V.Fecha AS DATE) <= CAST(@FechaHastaTAB AS DATE) AND P.ID = @IdProductoTAB
        ...
        SET @PREVIO_EXISTE_COSTEABLE = ISNULL((SELECT TOP 1 v.ROWid FROM ERP.KardexRecalculo v WHERE V.ROWid< @FilaROWid AND V.IdProducto= @FilaIdProducto AND V.FlagCostear= 1 
        ...
        BEGIN
            ...
            ELSE
            BEGIN
            SET @PRECIO_UNITARIO= (SELECT TOP 1 v.CalculadoPU FROM ERP.KardexRecalculo v WHERE V.ROWid< @FilaROWid AND V.IdProducto= @FilaIdProducto AND V.FlagCostear= 1 
     ...
     -------------------------------
        ...
        BEGIN
            ...
            SELECT TOP 1 @VALEORIGEN =isnull(vt.IdValeOrigen,0) , @ALMACENORIGEN = isnull(vt.IdAlmacenOrigen,0)  FROM ERP.ValeTransferencia vt INNER JOIN ERP.ValeTransferenciaDetalle vtd ON vt.ID = vtd.IdValeTransferencia
        ...
        SET @PREVIO_EXISTE_COSTEABLE = ISNULL((SELECT TOP 1 v.ROWid FROM ERP.KardexRecalculo v WHERE V.ROWid< @FilaROWid AND V.IdProducto= @FilaIdProducto AND V.FlagTraEntreAlm= 1 
                                ORDER BY v.ROWid DESC),-1)

        IF (@PREVIO_EXISTE_COSTEABLE = -1)
        BEGIN
            SET @PRECIO_UNITARIO= 1
            UPDATE ERP.KardexRecalculo SET  KardexRecalculo.CalculadoPU = @PRECIO_UNITARIO  WHERE ROWid =@PREVIO_EXISTE_COSTEABLE;
            PRINT 'UNO'
            END
            ELSE
            BEGIN
            SET @PRECIO_UNITARIO= (SELECT TOP 1 v.CalculadoPU FROM ERP.KardexRecalculo v WHERE V.ROWid< @FilaROWid AND V.IdProducto= @FilaIdProducto AND V.FlagTraEntreAlm= 1 
                                ORDER BY v.ROWid DESC)
            UPDATE ERP.KardexRecalculo SET  KardexRecalculo.CalculadoPU = @PRECIO_UNITARIO  WHERE ROWid = @FilaROWid;
