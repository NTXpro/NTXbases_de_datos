CREATE PROC [ERP].[Usp_Sel_ReporteBalance_Periodo] --4,8,1,9,1,'10'
@IdEmpresa INT,
@IdAnio INT,
@IdMesDesde INT,
@IdMesHasta INT,
...
BEGIN
	...
	SELECT
		...
		P.ID AS IdPeriodo,
		CONCAT(AN.Nombre, RIGHT('00' + LTRIM(RTRIM(M.Valor)), 2)) AS NombrePeriodo,
		ISNULL(SUM(CASE AD.IdDebeHaber WHEN 1 THEN (CASE WHEN @IdMoneda = 1 THEN CAST(AD.ImporteSoles AS DECIMAL(14, 2)) ELSE CAST(AD.ImporteDolares AS DECIMAL(14, 2)) END) END), 0) AS Debe,
		ISNULL(SUM(CASE AD.IdDebeHaber WHEN 2 THEN (CASE WHEN @IdMoneda = 1 THEN CAST(AD.ImporteSoles AS DECIMAL(14, 2)) ELSE CAST(AD.ImporteDolares AS DECIMAL(14, 2)) END) END), 0) AS Haber,
	...
	A.Flag = 1
	GROUP BY P.ID, AN.Nombre, M.Valor
	ORDER BY CAST(AN.Nombre AS INT), M.Valor ASC
END