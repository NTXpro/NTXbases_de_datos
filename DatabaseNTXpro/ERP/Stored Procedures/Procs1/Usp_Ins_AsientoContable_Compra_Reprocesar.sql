CREATE PROC [ERP].[Usp_Ins_AsientoContable_Compra_Reprocesar] --2553, 1723, 5, 4
@IdAsiento INT,
@IdComprobante INT,
@IdOrigen INT,
@IdSistema INT
...
------================ MODIFICAR CABECERA ================------
UPDATE ERP.Asiento SET Nombre = @NombreAsientoDetalle
WHERE ID = @IdAsiento
------====================================================------
...
DECLARE @OrdenAsiento40 INT = 1;
	...
	SELECT @IdAsiento,
		...
		CASE 
			WHEN C.Flag = 0 THEN 0
			WHEN C.IdMoneda = 1 THEN Total 
			ELSE CAST(Total * TipoCambio AS DECIMAL(14,2)) 
		END,
		CASE
			WHEN C.Flag = 0 THEN 0 
			WHEN C.IdMoneda = 2 THEN Total 
			ELSE CAST(Total / TipoCambio AS DECIMAL(14,2)) 
		END,
		@IdEntidadCliente,
		IdTipoComprobante,
...
BEGIN
	...
	SELECT @IdAsiento,
			...
			CASE
				WHEN C.Flag = 0 THEN 0  
				WHEN C.IdMoneda = 1 THEN IGV
				ELSE IGV * TipoCambio
			END,
			CASE
				WHEN C.Flag = 0 THEN 0
				WHEN C.IdMoneda = 2 THEN IGV
				ELSE IGV / TipoCambio
			END,
			@IdEntidadCliente,
			IdTipoComprobante,
...
BEGIN
	...
	CASE
		WHEN C.Flag = 0 THEN 0
		WHEN C.IdMoneda = 1 THEN @IR
		ELSE @IR * TipoCambio
	END,
	CASE 
		WHEN C.Flag = 0 THEN 0
		WHEN C.IdMoneda = 2 THEN @IR 
		ELSE @IR / TipoCambio
	END,
	@IdEntidadCliente,
	IdTipoComprobante,
...
BEGIN
	...
	CASE
		WHEN C.Flag = 0 THEN 0 
		WHEN C.IdMoneda = 1 THEN @RS
		ELSE @RS * TipoCambio
	END,
	CASE
		WHEN C.Flag = 0 THEN 0
		WHEN C.IdMoneda = 2 THEN @RS
		ELSE @RS / TipoCambio
	END,
	@IdEntidadCliente,
	IdTipoComprobante,
...
BEGIN
	...
	SELECT @IdAsiento,
		...
		CASE
			WHEN C.Flag = 0 THEN 0
			WHEN C.IdMoneda = 1 THEN ISC
			ELSE ISC * TipoCambio
		END,
		CASE 
			WHEN C.Flag = 0 THEN 0
			WHEN C.IdMoneda = 2 THEN ISC
			ELSE ISC / TipoCambio
		END,
		@IdEntidadCliente,
		IdTipoComprobante,
...
BEGIN
	...
	CASE
		WHEN C.Flag = 0 THEN 0
		WHEN C.IdMoneda = 1 THEN OtroImpuesto
		ELSE OtroImpuesto * TipoCambio
	END,
	CASE
		WHEN C.Flag = 0 THEN 0
		WHEN C.IdMoneda = 2 THEN OtroImpuesto
		ELSE OtroImpuesto / TipoCambio
	END,
	@IdEntidadCliente,
	IdTipoComprobante,
...
BEGIN
	...
	CASE
		WHEN C.Flag = 0 THEN 0
		WHEN C.IdMoneda = 1 THEN Descuento
		ELSE Descuento * TipoCambio
	END,
	CASE 
		WHEN C.Flag = 0 THEN 0
		WHEN C.IdMoneda = 2 THEN Descuento
		ELSE Descuento / TipoCambio
	END,
	@IdEntidadCliente,
	IdTipoComprobante,
...
BEGIN
	...
	CASE
		WHEN C.Flag = 0 THEN 0
		WHEN C.IdMoneda = 1 THEN RedondeoSuma
		ELSE RedondeoSuma * TipoCambio
	END,
	CASE
		WHEN C.Flag = 0 THEN 0
		WHEN C.IdMoneda = 2 THEN RedondeoSuma
		ELSE RedondeoSuma / TipoCambio
	END,
	@IdEntidadCliente,
	IdTipoComprobante,
...
SELECT @IdAsiento,
	...
	CASE
		WHEN C.Flag = 0 THEN 0 
		WHEN C.IdMoneda = 1 THEN RedondeoResta
		ELSE RedondeoResta * TipoCambio
	END,
	CASE
		WHEN C.Flag = 0 THEN 0
		WHEN C.IdMoneda = 2 THEN RedondeoResta
		ELSE RedondeoResta / TipoCambio
	END,
	@IdEntidadCliente,
	IdTipoComprobante,
...
BEGIN
	...
	SET @Orden = (SELECT TOP 1 MAX(Identificador) 
					FROM @TableAsientoDetalle 
					WHERE IdAsiento = @IdAsiento AND 
					IdDebeHaber != @IdDebeHaberTotal)

	UPDATE @TableAsientoDetalle 
	SET ImporteSoles = ImporteSoles + @TotalDiferencia 
	WHERE Identificador = @Orden;
END
------==============================================================------
...
SELECT
	...
	AD.Nombre,
	CASE WHEN AD.IdTipoComprobante = 8 THEN
