CREATE PROCEDURE [ERP].[Usp_Sel_CobranzaDocumentoCuentaCobrar] 
@TipoFecha INT,
@EstadoDocumento INT,
@Fecha DATETIME,
@FechaHasta datetime,
	...
	(SELECT	ENT.Nombre											Cliente,
			...
			(CASE
				WHEN CC.IdMoneda = 1 AND CC.IdDebeHaber = 1 THEN
						(SELECT([ERP].[SaldoTotalCobranzaCuentaCobrarDeSolesPorFecha](CC.ID, CAST('9999-12-31 23:59:59.997' AS DateTime))))
				WHEN CC.IdMoneda = 1 AND CC.IdDebeHaber = 2 THEN
						(SELECT([ERP].[SaldoTotalCobranzaCuentaCobrarDeSolesPorFecha](CC.ID, CAST('9999-12-31 23:59:59.997' AS DateTime)))) * (-1)
				WHEN CC.IdMoneda = 2 AND CC.IdDebeHaber = 1 THEN
						(SELECT([ERP].[SaldoTotalCobranzaCuentaCobrarDeDolaresPorFecha](CC.ID, CAST('9999-12-31 23:59:59.997' AS DateTime))))
				WHEN CC.IdMoneda = 2 AND CC.IdDebeHaber = 2 THEN
						(SELECT([ERP].[SaldoTotalCobranzaCuentaCobrarDeDolaresPorFecha](CC.ID, CAST('9999-12-31 23:59:59.997' AS DateTime)))) * (-1)
					END) AS Saldo,
			(CASE
		...
		WHERE (
				(@TipoFecha = 1 AND CAST(ST.FechaEmision AS DATE) <= CAST(@Fecha AS DATE)) OR
				(@TipoFecha = 2 AND CAST(ST.FechaVencimiento AS DATE) BETWEEN CAST(@Fecha AS DATE) AND CAST(@FechaHasta AS DATE))
			   )
