...
CREATE PROC [ERP].[Usp_Paginate_DocumentoPagar]
...
WITH LISTA_TEMP AS (
	...
	IIF(MT.IdMoneda = 1 ,MTD.Total,MTD.Total * CP.TipoCambio)*IIF(CP.IdTipoComprobante IN (8,55,60,183,178),-1,1) TotalMovimientoConvSoles,
	IIF(MT.IdMoneda = 2 ,MTD.Total,MTD.Total/CP.TipoCambio)*IIF(CP.IdTipoComprobante IN (8,55,60,183,178),-1,1) as TotalMovimientoConvDolares,
	...
	INNER JOIN ERP.CuentaPagar CP ON CP.ID = MDCP.IdCuentaPagar AND CP.IdTipoComprobante NOT IN (183)
	INNER JOIN ERP.Entidad ENT ON ENT.ID = CP.IdEntidad
	INNER JOIN ERP.Proveedor PRO ON PRO.IdEntidad = ENT.ID
