...
CREATE PROC [ERP].[Usp_Ins_SaldoInicial]
@ID INT OUT,
@IdEmpresa INT,
@IdProveedor INT,
...
AS
BEGIN
		DECLARE @IdPeriodo INT = (SELECT [ERP].[ObtenerIdPeriodo_By_Fecha](@Fecha))
		INSERT INTO ERP.SaldoInicial(IdEmpresa,IdProveedor,IdTipoComprobante,IdMoneda,Fecha,FechaVencimiento,Serie,Documento,Monto,TipoCambio,Flag,FlagBorrador,FechaRegistro,UsuarioRegistro,FechaRecepcion)
							 VALUES(@IdEmpresa,
							 (CASE  @IdProveedor WHEN 0 THEN NULL ELSE @IdProveedor END),
							 @IdTipoComprobante,
							 @IdMoneda,
							 @Fecha,
							 @FechaVencimiento,
