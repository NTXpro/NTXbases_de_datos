CREATE PROC [ERP].[Usp_Ins_Empresa]
@IdTipoActividad		INT,
@IdEmpresa				INT OUT,
@IdEntidad				INT OUT,
@IdTipoPersona			INT,
...
BEGIN
		...
		INSERT INTO ERP.Empresa (IdEntidad,IdTipoActividad, NombreImagen, Imagen, NumeroResolucion,Correo,Web,Telefono,Celular, UsuarioRegistro,FechaRegistro,FlagBorrador,Flag)
		VALUES(@IdEntidad, @IdTipoActividad, @NombreImagen, @Imagen, @NumeroResolucion,@Correo,@Web,@Telefono,@Celular, @UsuarioRegistro, DATEADD(HOUR, 3, GETDATE()),@FlagBorrador, 1)

		SET @IdEmpresa = (SELECT CAST(SCOPE_IDENTITY() AS INT));
