...
CREATE PROC [ERP].[Usp_Ins_Vendedor]
@IdVendedor				INT OUT,
@IdEntidad				INT OUT,
@IdEmpresa				INT,
@IdTrabajador			INT,
...
BEGIN
		...
		IF @IdTrabajador = 0
		BEGIN
			INSERT INTO ERP.Trabajador (IdEntidad,IdEmpresa,FechaRegistro,FlagBorrador,Flag) 
			VALUES (@IdEntidad,@IdEmpresa,DATEADD(HOUR, 3, GETDATE()),@FlagBorrador,1)

			SET @IdTrabajador = (SELECT CAST(SCOPE_IDENTITY() AS INT));
		END
		

		INSERT INTO ERP.Vendedor (IdTrabajador,UsuarioRegistro,FechaRegistro,FlagBorrador,Flag)
		VALUES(@IdTrabajador, @UsuarioRegistro,DATEADD(HOUR, 3, GETDATE()),@FlagBorrador, 1)

		SET @IdVendedor = (SELECT CAST(SCOPE_IDENTITY() AS INT));
