create PROC [ERP].[Usp_Sel_AnticipoCompra_Inactivo]
@IdEmpresa INT,
@IdAnio INT,
@IdMes INT
AS
BEGIN
		...
		WHERE AN.Flag = 0 AND AN.FlagBorrador = 0 AND AN.IdEmpresa = @IdEmpresa AND YEAR(AN.FechaEmision) = @IdAnio AND MONTH(AN.FechaEmision) = @IdMes
