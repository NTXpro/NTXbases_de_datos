...
create PROCEDURE [ERP].[Usp_Ins_AjusteCuentaPagar]
@XMLAjuste	 XML
AS
BEGIN
		...
		DECLARE @UltimoDocumentoGenerado INT= ISNULL((SELECT MAX(Documento) FROM ERP.AjusteCuentaPagar WHERE IdEmpresa = @IdEmpresa),0);


		INSERT INTO [ERP].[AjusteCuentaPagar]
		 (
			 IdTipoComprobante
			,IdCuentaPagar
			,IdEntidad
