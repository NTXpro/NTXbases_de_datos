CREATE PROCEDURE [ERP].[Usp_Upd_Cotizacion]
@IdCotizacion	 INT,
@Serie			 VARCHAR(4),
@IdEmpresa		 INT,
@XMLCotizacion	 XML
...
SET QUERY_GOVERNOR_COST_LIMIT 36000
		...
		DECLARE @FlagControlaStock BIT = (SELECT T.N.value('FlagControlarStock[1]','BIT') FROM @XMLCotizacion.nodes('/ArchivoPlanoCotizacion') AS T(N));
		DECLARE @FlagGenerarVale BIT = (SELECT T.N.value('FlagGenerarVale[1]','BIT') FROM @XMLCotizacion.nodes('/ArchivoPlanoCotizacion') AS T(N));
		DECLARE @FlagBorrador BIT = (SELECT FlagBorrador FROM ERP.Cotizacion WHERE ID = @IdCotizacion)
		...
		IF((SELECT T.N.value('FlagBorrador[1]','BIT') FROM @XMLCotizacion.nodes('/ArchivoPlanoCotizacion') AS T(N)) = 0 ) 
			BEGIN
				...
				IF ((SELECT T.N.value('Documento[1]','VARCHAR(20)') FROM @XMLCotizacion.nodes('/ArchivoPlanoCotizacion') AS T(N)) = '' OR
				(SELECT T.N.value('Documento[1]','VARCHAR(20)') FROM @XMLCotizacion.nodes('/ArchivoPlanoCotizacion') AS T(N)) IS NULL OR
				(SELECT T.N.value('Documento[1]','VARCHAR(20)') FROM @XMLCotizacion.nodes('/ArchivoPlanoCotizacion') AS T(N)) = '00000000')
				BEGIN
					SET @Documento = (SELECT [ERP].[GenerarDocumentoCotizacion](@Serie,@IdCotizacion,@IdEmpresa));
				END
				ELSE
				BEGIN
					SET @Documento = (SELECT T.N.value('Documento[1]','VARCHAR(20)') FROM @XMLCotizacion.nodes('/ArchivoPlanoCotizacion') AS T(N));
				END
		...
		IF @FlagBorrador = 0
			BEGIN
				SET @Documento =  (SELECT Documento FROM ERP.Cotizacion WHERE ID = @IdCotizacion)
			END
		...
		DELETE FROM [ERP].[CotizacionDetalle] WHERE IdCotizacion = @IdCotizacion
	...
	FROM @XMLCotizacion.nodes('/ListaArchivoPlanoCotizacionDetalle/ArchivoPlanoCotizacionDetalle') AS T(N)	
		...
		DELETE FROM [ERP].[CotizacionReferencia] WHERE IdCotizacion = @IdCotizacion
