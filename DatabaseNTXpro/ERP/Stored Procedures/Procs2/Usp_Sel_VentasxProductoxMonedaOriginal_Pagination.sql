CREATE PROC [ERP].[Usp_Sel_VentasxProductoxMonedaOriginal_Pagination] --1,'2017-01-01','2017-01-31',1,100,'','',5
@IdEmpresa INT ,
...
BEGIN
		...
		WITH VentasProductoOriginal AS
		(
		...
		AS ROWNUMBER,
					...
					LEFT JOIN 
						...
						GROUP BY P.Nombre,CD.Cantidad,T6.Nombre,T10.CodigoSunat,C.Serie,C.Documento,C.Fecha

			)
			,COUNT_VENTASPRODUCTO AS (
					SELECT COUNT(Nombre) AS [TotalCount]
					FROM VentasProductoOriginal
				)
				SELECT			
								Nombre,
								Cantidad,
								...
								Porcentaje,
								TotalCount
				FROM 	VentasProductoOriginal, COUNT_VENTASPRODUCTO
