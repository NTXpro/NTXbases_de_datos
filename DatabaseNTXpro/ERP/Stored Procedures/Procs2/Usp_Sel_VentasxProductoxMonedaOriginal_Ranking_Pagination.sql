CREATE PROC [ERP].[Usp_Sel_VentasxProductoxMonedaOriginal_Ranking_Pagination] --1,'2017-01-01','2017-01-31',1,123,'Nombre','DESC',5
@IdEmpresa INT ,
...
BEGIN
		...
		WITH VentasProductoRankingOriginal AS
		(
			...
			(ORDER BY 
					...
					--ASC
						...
						CASE WHEN (@SortType = 'TotalConvPen' AND @SortDir = 'ASC') THEN T6.Nombre END ASC,
						--CASE WHEN (@SortType = 'TotalPEN' AND @SortDir = 'ASC') THEN EN.Nombre END ASC,
						
						--DESC
						CASE WHEN (@SortType = 'Nombre' AND @SortDir = 'DESC') THEN P.Nombre END DESC,
						CASE WHEN (@SortType = 'UnidadMedida' AND @SortDir = 'DESC') THEN T6.Nombre END DESC,
						CASE WHEN (@SortType = 'TotalConvPen' AND @SortDir = 'DESC') THEN T6.Nombre END DESC
		...
		AS ROWNUMBER,
			...
			,COUNT_VENTASPRODUCTO_RANKING AS (
					SELECT COUNT(Nombre) AS [TotalCount]
					FROM VentasProductoRankingOriginal
				)
				SELECT			
								...
								Porcentaje,
								TotalCount
				FROM 	VentasProductoRankingOriginal, COUNT_VENTASPRODUCTO_RANKING
