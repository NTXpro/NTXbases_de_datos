CREATE PROC [ERP].[Usp_Sel_VentasxVendedorxMonedaOriginal_Ranking_Pagination] --1,'2017-01-01','2017-01-31',1,50,'RUC','DESC',5
@IdEmpresa INT ,
...
BEGIN
		...
		WITH VentasVendedorRankingOriginal AS
		(
		...
		AS ROWNUMBER,
			...
			,COUNT_VENTASVENDEDOR_RANKING AS (
					SELECT COUNT(NombreVendedor) AS [TotalCount]
					FROM VentasVendedorRankingOriginal
				)
				SELECT			
								...
								Porcentaje,
								TotalCount
				FROM 	VentasVendedorRankingOriginal, COUNT_VENTASVENDEDOR_RANKING
