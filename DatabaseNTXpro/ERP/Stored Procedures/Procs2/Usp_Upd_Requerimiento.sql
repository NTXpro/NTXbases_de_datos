CREATE PROCEDURE [ERP].[Usp_Upd_Requerimiento]
@IdRequerimiento	 INT,
...
BEGIN
		...
		IF @FlagBorrador = 0
			BEGIN
				SET @Documento =  (SELECT Documento FROM ERP.Requerimiento WHERE ID = @IdRequerimiento)
			END

		UPDATE ERP.Requerimiento SET 
		...
		DELETE FROM RequerimientoDetalle WHERE IdRequerimiento = @IdRequerimiento
		...
		DELETE FROM [ERP].[RequerimientoReferencia] WHERE IdRequerimiento = @IdRequerimiento
