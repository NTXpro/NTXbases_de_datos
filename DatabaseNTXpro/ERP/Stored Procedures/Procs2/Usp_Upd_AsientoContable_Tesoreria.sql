...
CREATE PROC [ERP].[Usp_Upd_AsientoContable_Tesoreria] 
@IdAsiento INT,
@IdMovimiento INT,
@IdOrigen INT,
@IdSistema INT
...
UPDATE
    TableAsiento
SET
    TableAsiento.Nombre = @NombreAsiento,
	TableAsiento.IdMoneda = TableCuenta.IdMoneda,
	TableAsiento.Fecha = TableMovimiento.Fecha,
	TableAsiento.TipoCambio = TableMovimiento.TipoCambio,
	TableAsiento.Referencia = TableCuenta.Nombre + '-'+cast(TableMovimiento.Orden AS VARCHAR(20))
FROM
    (ERP.Asiento AS TableAsiento
    INNER JOIN ERP.MovimientoTesoreria AS TableMovimiento ON TableMovimiento.IdAsiento = TableAsiento.ID
	LEFT JOIN Maestro.CategoriaTipoMovimiento CTM ON CTM.ID = TableMovimiento.IdCategoriaTipoMovimiento
	LEFT JOIN Maestro.TipoMovimiento TM ON TM.ID = TableMovimiento.IdTipoMovimiento
	INNER JOIN ERP.Cuenta AS TableCuenta ON TableCuenta.ID = TableMovimiento.IdCuenta)
WHERE
    TableAsiento.ID = @IdAsiento
------========================================================------
