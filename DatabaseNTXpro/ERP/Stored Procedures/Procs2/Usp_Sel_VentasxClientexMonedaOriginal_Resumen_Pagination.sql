CREATE PROC [ERP].[Usp_Sel_VentasxClientexMonedaOriginal_Resumen_Pagination] --1,'2017-01-01','2017-01-31',1,100,'RUC','DESC',5
@IdEmpresa INT ,
...
BEGIN
		...
		WITH VentasClienteResumenOriginal AS
		(
			...
			(ORDER BY 
					...
					--ASC
						...
						CASE WHEN (@SortType = 'TotalConvPen' AND @SortDir = 'ASC') THEN sum (iif(M.CodigoSunat = 'PEN',C.Total,C.Total*iif(T10.CodigoSunat='07',RF.VentaSunat,C.TipoCambio))*iif(T10.CodigoSunat='07',-1,1))  END ASC,
						--CASE WHEN (@SortType = 'RUC' AND @SortDir = 'ASC') THEN ETD.NumeroDocumento END ASC,
						...
						CASE WHEN (@SortType = 'TotalConvPen' AND @SortDir = 'DESC') THEN sum (iif(M.CodigoSunat = 'PEN',C.Total,C.Total*iif(T10.CodigoSunat='07',RF.VentaSunat,C.TipoCambio))*iif(T10.CodigoSunat='07',-1,1)) END DESC
						--CASE WHEN (@SortType = 'RUC' AND @SortDir = 'DESC') THEN ETD.NumeroDocumento END DESC
		...
		AS ROWNUMBER,
			...
			,COUNT_VENTASCLIENTE_RESUMEN AS (
					SELECT COUNT(RUC) AS [TotalCount]
					FROM VentasClienteResumenOriginal
				)
				SELECT			RUC,
								RazonSocial,
								SubTotalPEN,
								...
								Porcentaje,
								TotalCount
				FROM 	VentasClienteResumenOriginal, COUNT_VENTASCLIENTE_RESUMEN
