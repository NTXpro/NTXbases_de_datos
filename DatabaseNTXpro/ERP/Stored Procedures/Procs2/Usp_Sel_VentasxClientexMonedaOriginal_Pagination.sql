CREATE PROC [ERP].[Usp_Sel_VentasxClientexMonedaOriginal_Pagination]  --1,'2016-01-01','2016-12-03',1,10000,'','',5
@IdEmpresa INT ,
...
BEGIN
		...
		WITH VentasClienteOriginal AS
		(
			SELECT ROW_NUMBER() OVER
			(ORDER BY 

					--ASC
						CASE WHEN (@SortType = 'Documento' AND @SortDir = 'ASC') THEN T2.Abreviatura END ASC,
						...
						--DESC
						CASE WHEN (@SortType = 'Documento' AND @SortDir = 'DESC') THEN T2.Abreviatura END DESC,
		...
		AS ROWNUMBER,
					...
					--	ON C.Fecha = TC.Fecha
					LEFT JOIN 
						(SELECT CM.ID, C.Fecha, T10.CodigoSunat, C.Serie, C.Documento, C.TipoCambio FROM ERP.ComprobanteReferenciaInterno R
						...
						ON C.ID = RF.ID
						WHERE C.IdComprobanteEstado <> 3 AND NOT C.FlagBorrador = 1 AND C.IdEmpresa = @IdEmpresa AND C.Fecha BETWEEN @FechaInicio AND @FechaFin
						GROUP BY EN.Nombre, T2.Abreviatura, ETD.NumeroDocumento, 
						T10.CodigoSunat, C.Serie, C.Documento, C.Fecha
						--ORDER BY RazonSocial,Documento
			)
			,COUNT_VENTASCLIENTE AS (
					SELECT COUNT(Documento) AS [TotalCount]
					FROM VentasClienteOriginal
				)
				SELECT			Documento,
								RUC,
								RazonSocial,
								...
								Porcentaje,
								TotalCount
				FROM 	VentasClienteOriginal, COUNT_VENTASCLIENTE
