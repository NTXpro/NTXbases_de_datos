CREATE FUNCTION [ERP].[SaldoTotalCuentaCobrarDeDolaresPorFecha] (@IdCuentaCobrar INT,@Fecha DATETIME)
...
BEGIN
			...
			SET @TotalMovimientoCuentaCobrar = (SELECT SUM (CASE WHEN MT.IdMoneda = 2 AND CC.IdMoneda = 2 THEN
																		MTD.Total
														...
														WHERE CC.ID = @IdCuentaCobrar AND MT.Flag = 1 AND MT.FlagBorrador = 0 AND CAST(MT.Fecha AS DATE) <= CAST(@Fecha AS DATE) 
														/*AND CC.IdTipoComprobante NOT IN (183)*/
														)

			SET @TotalAjuste = (SELECT SUM(CASE WHEN A.IdMoneda = 2  THEN
								A.Total
							WHEN A.IdMoneda = 1 THEN
								ROUND((A.Total / A.TipoCambio),2)
							END) 
					FROM ERP.AjusteCuentaCobrar A
					WHERE A.IdCuentaCobrar = @IdCuentaCobrar AND CAST(A.Fecha AS DATE) <= CAST(@Fecha AS DATE))
			...
			IF(@IdTipoComprobanteCuentaCobrar IN (SELECT ID FROM @IdTipoComprobante))
				...
				SET @TotalAplicacionAnticipo = (SELECT SUM (AACD.TotalAplicado)
															...
															WHERE CC.ID = @IdCuentaCobrar AND CAST(AAC.FechaRegistro AS DATE) <= CAST(@Fecha AS DATE))
				END
			ELSE
				BEGIN
				SET @TotalAplicacionAnticipo = (SELECT SUM (CASE WHEN AAC.IdMoneda = 2 AND CC.IdMoneda = 2 THEN
																		AACD.TotalAplicado
															WHEN AAC.IdMoneda = 1 AND CC.IdMoneda = 2 THEN
