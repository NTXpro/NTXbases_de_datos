CREATE FUNCTION [ERP].[SaldoTotalCuentaPagarDeDolaresPorFecha](@IdCuentaPagar INT, @Fecha DATETIME)
...
DECLARE @TotalAplicacionAnticipoDetalle DECIMAL(14,5);
...
SET @TotalMovimientoCuentaPagar  = (SELECT SUM(CASE WHEN MT.IdMoneda = 2 AND CP.IdMoneda = 2 THEN 
																		MTD.Total
...
ELSE
	BEGIN
		SET @TotalAplicacionAnticipo	=	(SELECT SUM(CASE
															WHEN AAP.IdMoneda = 2 AND CP.IdMoneda = 2
																THEN AAPD.TotalAplicado
															WHEN AAP.IdMoneda = 1 AND CP.IdMoneda = 2
