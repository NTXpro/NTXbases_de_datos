...
CREATE FUNCTION [ERP].[ObtenerTotalSaldoFinalMovimientoByCuentaPeriodo](
@IdEmpresa INT,
@IdCuenta INT,
@Mes INT,
@Anio INT
...
BEGIN
	...
	DECLARE @SaldoInicialCuenta DECIMAL(14,5) = (SELECT [ERP].[ObtenerTotalSaldoInicialMovimientoByCuentaPeriodo](@IdEmpresa, @IdCuenta, @Mes, @Anio))

	DECLARE @TotalIngresoMovimiento DECIMAL(14,5) = (SELECT SUM(CASE WHEN @IdCuenta = 0 AND MT.IdMoneda = 2 THEN ---SI LA CUENTA ES TODOS LOS MONTOS DEBEN SER EN SOLES
	...
	DECLARE @TotalSalidaMovimiento DECIMAL(14,5) = (SELECT SUM(CASE WHEN @IdCuenta = 0 AND MT.IdMoneda = 2 THEN ---SI LA CUENTA ES TODOS LOS MONTOS DEBEN SER EN SOLES
																	MT.Total / MT.TipoCambio
																ELSE
																	MT.Total
																END)
