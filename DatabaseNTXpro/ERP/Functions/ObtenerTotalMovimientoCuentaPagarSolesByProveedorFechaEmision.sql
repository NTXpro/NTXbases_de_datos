...
CREATE FUNCTION [ERP].[ObtenerTotalMovimientoCuentaPagarSolesByProveedorFechaEmision](
		@IdProveedor INT,
		...
		BEGIN
			...
			SET  @TotalCuentaPagarSoles = (SELECT 
											SUM(IIF(CP.IdMoneda = 1 ,CP.Total,'0.0000')*IIF(CP.IdTipoComprobante = 8,-1,1))
											FROM ERP.CuentaPagar CP
											INNER JOIN ERP.Entidad ENT ON ENT.ID = CP.IdEntidad
											INNER JOIN ERP.Proveedor PRO ON PRO.IdEntidad = ENT.ID
			...
			SET  @TotalMovimientoCuentaPagarSoles = (SELECT 
													 SUM(IIF(CP.IdMoneda = 1 ,MTD.Total,'0.0000')*IIF(CP.IdTipoComprobante = 8,-1,1))
												     ...
												     WHERE PRO.ID = @IdProveedor AND MT.Flag = 1 
													 AND MT.FlagBorrador = 0 
													 AND MTD.IdDebeHaber = 1 
													 AND CP.IdTipoComprobante IN (SELECT DATA FROM [ERP].[Fn_SplitContenido] (@ListaTipoComprobante,','))
