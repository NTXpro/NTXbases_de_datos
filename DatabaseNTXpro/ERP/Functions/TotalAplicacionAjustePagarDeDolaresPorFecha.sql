CREATE FUNCTION [ERP].[TotalAplicacionAjustePagarDeDolaresPorFecha](@IdCuentaPagar INT, @Fecha DATETIME) 
			...
			SET @TotalAjuste = (SELECT SUM(CASE WHEN A.IdMoneda = 2 THEN
								...
								WHERE A.IdCuentaPagar = @IdCuentaPagar AND CAST(A.Fecha AS DATE) <= CAST(@Fecha AS DATE) AND C.IdMoneda = 2)
			...
			IF(@IdTipoComprobanteCuentaPagar IN (SELECT ID FROM @IdTipoComprobante))
				...
				SET @TotalAplicacionAnticipo	=  (SELECT SUM (AAPD.TotalAplicado)
																...
																WHERE CP.ID = @IdCuentaPagar AND CAST(AAP.FechaAplicacion AS DATE) <= CAST(@Fecha AS DATE) AND CP.IdMoneda = 2 AND AAP.Flag = 1);
				END
			ELSE
				BEGIN
				SET @TotalAplicacionAnticipo	= (SELECT SUM(CASE WHEN AAP.IdMoneda = 2 THEN
																			AAPD.TotalAplicado
															   WHEN AAP.IdMoneda = 1 THEN 
																...
																WHERE CP.ID = @IdCuentaPagar AND CAST(AAP.FechaAplicacion AS DATE) <= CAST(@Fecha AS DATE) AND CP.IdMoneda = 2
																AND AAP.Flag = 1)
