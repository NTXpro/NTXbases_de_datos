CREATE FUNCTION [ERP].[ObtenerTotalMovimientoCuentaCobrarSolesByClienteFechaEmision](
@IdCliente INT,
...
BEGIN
	...
	SET @TotalCuentaCobrarSoles = (SELECT SUM(IIF(CC.IdMoneda = 1 ,CC.Total,'0.0000')*IIF(CC.IdTipoComprobante = 8,-1,1))
									...
									INNER JOIN ERP.Cliente CLI ON CLI.IdEntidad = ENT.ID AND CC.IdEmpresa = CLI.IdEmpresa
	...
	SET @TotalAnticipo = (SELECT SUM(IIF(AAC.IdMoneda = 1 ,AAC.Total,'0.0000'))
	...
	SET @TotalAnticipoDetalle = (SELECT SUM(IIF(AACC.IdMoneda = 1 ,AACC.TotalAplicado,'0.0000'))
											FROM ERP.AplicacionAnticipoCobrar AAC
