...
CREATE FUNCTION [ERP].[ObtenerTotalMovimientoByCuentaFecha](
@IdCuenta INT,
@Fecha DATETIME
)
...
BEGIN
	...
	SET @TotalMovimientoIngreso = (SELECT SUM(MT.Total) 
									...
									AND Flag = 1 AND FlagBorrador = 0 AND IdTipoMovimiento = 1);

	SET @TotalMovimientoSalida = (SELECT SUM(MT.Total) 
									FROM ERP.MovimientoTesoreria MT
									WHERE IdCuenta = @IdCuenta AND CAST(Fecha AS DATE) <= CAST(@Fecha AS DATE)
									AND Flag = 1 AND FlagBorrador = 0 AND IdTipoMovimiento = 2);
